...
class DeUpBlock(nn.Module):
    ...
    def __init__(self, inf, onf):
        ...
        sequence = [
            nn.ConvTranspose3d(inf, onf, kernel_size=6, stride=2, padding=2),
            lrelu,
        ]
...
class ResnetGenerator(nn.Module):
    def __init__(
        ...
        self.conv_blockl_1 = nn.Sequential(
            nn.Conv3d(input_nc, ngf, kernel_size=3, padding=1), lrelu
        )
        # residual blocks
        for i in range(self.n_residual_blocks):
            self.add_module("residual_block" + str(i + 1), ResnetBlock(ngf, ngf))
        # the conv after residual blocks
        self.conv_blockl_2 = nn.Sequential(
            nn.Conv3d(ngf, ngf, kernel_size=3, padding=1), nn.BatchNorm3d(ngf)
        )
        # upsample blocks
